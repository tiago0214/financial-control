...
<template>
  <!-- Card Container -->
  <div
    class="flex flex-col justify-between gap-6 rounded-3xl border border-border/50 bg-gradient-card p-6 shadow-card"
  >
    <!-- Header -->
    <header class="flex justify-between items-start">
      ...
      <span
        class="text-primary font-medium rounded-full bg-primary/10 px-3 py-1 text-xs"
      >
        {{ goal.duration }}
      </span>
    </header>
    ...
    <div class="flex flex-col gap-2">
      ...
      <div class="flex justify-between items-end">
        <span
          class="text-3xl font-bold text-foreground leading-none font-display"
          >{{ formatCurrency(goal.targetAmount) }}</span
        >
        <!-- Progress Details -->
        <div class="flex justify-between items-center text-sm mt-1">
          <span class="text-muted-foreground font-display"
            >{{ goal.progress }}% completo</span
          >
        </div>
      </div>
    ...
    <div
      v-if="goal.aiInsight"
      class="flex flex-col gap-2 rounded-2xl bg-primary/5 p-4"
    >
    ...
    <footer class="flex justify-between items-center mt-1">
      ...
      <button
        class="rounded-sm bg-gradient-primary px-4 py-2 font-semibold text-primary-foreground shadow-soft transition-all hover:shadow-glow cursor-pointer text-xs"
        @click="handleEdit"
      >
        Ajustar Meta
      </button>
