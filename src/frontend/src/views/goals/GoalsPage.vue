...
const goalsList = computed(() => {
  ...
  return goalsStore.userGoals.map(g => ({
    ...
    progress: g.targetAmount ? Math.min(100, Math.round((available / g.targetAmount) * 100)) : 0,
...
<template>
  <main class="grid grid-cols-1 space-y-6">
      <OverallGoals />
      <div v-if="goalsList.length > 0" class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
