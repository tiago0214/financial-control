...
function formatDate(dateStr: string) {
  ...
  const m = new Intl.DateTimeFormat('pt-BR', { month: 'short' }).format(dateObj);
...
const totalSaved = computed(() => transactionsStore.totalAvailable);

const goals = computed(() => {
  const available = transactionsStore.totalAvailable;
  return goalsStore.userGoals.map(g => {
    const progress = g.targetAmount ? Math.min(100, Math.round((available / g.targetAmount) * 100)) : 0;
    return {
      id: g.id,
      name: g.title,
  ...
  <section class="rounded-3xl border border-border/50 bg-gradient-card p-6 shadow-card">
    <div class="mb-5 flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-muted-foreground">{{ activeGoals }} ativos • {{ formatCurrency(totalSaved) }} economizados</p>
      </div>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="flex items-center gap-4">
          <div :class="['flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br shadow-soft', g.tint]">
            <component :is="g.icon" class="h-5 w-5 text-primary-foreground" :stroke-width="2.5" />
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center justify-between gap-2">
              <div class="font-semibold">{{ g.name }}</div>
              <div class="text-sm font-bold tabular-nums">{{ g.progress }}%</div>
            </div>
            ...
            <div class="mt-2 h-2 overflow-hidden rounded-full bg-secondary">
              <div
                :class="['h-full rounded-full bg-linear-to-r transition-all', g.tint]"
      ...
      <div v-if="goals.length === 0" class="text-center py-4 text-sm text-muted-foreground">
