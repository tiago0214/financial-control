...
const getIcon = (category: string) =>
  categoryIconMap[category] || CircleDollarSign;

const getTone = (status: string) =>
  status === "credito" ? "primary" : "alert";
...
<template>
  ...
  >
    <header class="text-lg font-semibold mb-4 pt-6 pl-6">
    ...
    <table class="w-full text-left border-collapse">
      <thead>
        <tr class="border-b border-border/50">
          <th class="py-3 font-medium text-muted-foreground text-sm pl-6">
            Descrição
          </th>
          <th class="py-3 font-medium text-muted-foreground text-sm">
            Categoria
          </th>
          ...
          <th
            class="py-3 font-medium text-muted-foreground text-sm text-right pr-6"
          >
            Valor
          </th>
        </tr>
      ...
      <tbody>
        <tr
          v-for="t in userTransactions"
          :key="t.id"
          @click="handleEdit(t.id)"
          class="group border-b border-border/50 last:border-0 hover:bg-white/5 transition-colors hover:cursor-pointer"
        >
          <td class="py-3 pl-6">
          ...
          <td class="py-3 text-right pr-6">
            <span
              :class="[
                ...
                t.status === 'credito' ? 'text-success' : 'text-accent',
              ]"
            >
              {{ t.status === "credito" ? "+" : "-" }}R$
